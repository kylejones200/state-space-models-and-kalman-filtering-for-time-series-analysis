...
def kalman_filter(y: np.ndarray, F: np.ndarray, H: np.ndarray, Q: np.ndarray, R: np.ndarray, x0: np.ndarray, P0: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
...
def plot_kalman_filter(y: np.ndarray, x_filtered: np.ndarray, title: str, output_path: Path, plot: bool=False):
